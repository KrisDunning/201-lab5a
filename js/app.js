...
function sumArray(sumArr) { //eslint-disable-line
  let total=sum(sumArr[0],sum(sumArr[1],sumArr[2])[0])[0];
...
function multiplyArray(multArr) { //eslint-disable-line
  let total=multiply(multArr[0],multiply(multArr[1],multArr[2])[0])[0];
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  myString='The numbers ';
  for(let i=0;i<arr.length;i++){
    myString+=`${arr[i]},`;
  }
  myString=myString.slice(0,-1);
  myString+=` have a product of ${product}.`;

  return [product,myString];
}
